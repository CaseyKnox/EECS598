...
def svm_loss_vectorized(
    ...
    N = y.shape[0]
    idx = torch.arange(N)
    y_hat = (W.T @ X.T) # (D,C) @ (N,D) -> (C, N) (transposed)
    correct_vec = y_hat[y, idx]
    margins = y_hat - correct_vec + 1 # broadcast (C,N) - (N,)
    margins = margins.clamp(min=0) # hinge loss
    margins[y, idx] = 0 # zero out the correct class
    loss = margins.sum() / N
    loss += reg * torch.sum(W*W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    ...
    # loss.                                                                     #
    #############################################################################
    # binary mask of margins for picking out from X
    M = (margins > 0).to(torch.double) # (C,N)
    sampl_sum = M.sum(dim=0) # (N,)
    M[y,idx] -= sampl_sum # encode subtractions in mask
    dW = X.T @ M.T # (N,D) (C,N) => (D,C)
    dW = dW / N
    dW += 2 * reg * W
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    logits = W.T @ X.T # (C,N)
    y_pred = torch.argmax(logits, dim=0)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def test_one_param_set(
    ...
    # num_iters = 100
    loss_history = cls.train(data_dict["X_train"], 
                             data_dict["y_train"],
                             lr,
                             reg,
                             num_iters)
    ...
    train_acc = torch.sum(y_pred == data_dict["y_train"]) / y_pred.shape[0]

    # predict on validation data
    y_pred_val = cls.predict(data_dict["X_val"])
    val_acc = torch.sum(y_pred_val == data_dict["y_val"]) / y_pred_val.shape[0]
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################

    return cls, train_acc.item(), val_acc.item()
...
def softmax_loss_naive(
    ...
    for i in range(N):
        xi = X[i] # (D,)
        logits = W.T @ xi # (C,) : class scores
        lmax = torch.max(logits)
        logits -= lmax # subtract the max to avoid numerical issues
        logits_exp = torch.exp(logits)
        S = logits_exp.sum() # (1,)
        probs = logits_exp / S # (C,)
        loss -= torch.log(probs[y[i]])

        for c in range(C):
            coeff = probs[c] - (1 if c == y[i] else 0)
            dW[:, c] += coeff * xi
...
def softmax_loss_vectorized(
    ...
    D, C = W.shape
    N, D = X.shape
    logits = W.T @ X.T # (C, N) = (D,C) @ (N,D)
    maxes = torch.max(logits, dim=0, keepdim=True).values
    logits -= maxes # (C,N)
    l_exp = logits.exp() # (C,N)
    S = l_exp.sum(axis=0) # along the C (N,)
    probs = l_exp / S # (C,N)
    log_probs = -1.0 * probs.log() # (C,N)
    loss = log_probs[y, torch.arange(N)].mean() # (1,)
    loss += reg * torch.sum(W * W)

    # backprob (kinda)
    # dlogprobs = torch.ones((C,)) * -1.0 / N # dl/dlogprobs 
    # dprobs = 1.0 / probs * dlogprobs # dlogprobs/dprobs * dlogprobs
    # dS = l_exp / (S * S) * dprobs # dprobs/dS
    # dl_exp = 1 / S * dprobs # dprobs/dl_exp
    # dl_exp += torch.ones_like(l_exp) * dprobs
    # dlogits = l_exp * dl_exp
    # dW = X - X[torch.argmax(logits)] * dlogits
    dlogits = probs.clone() # P_i
    dlogits[y, torch.arange(N)] -= 1 # P_i - 1
    dlogits /= N

    dW = X.T @ dlogits.T # (D,C) = (D,N) @ (C,N)
    dW += 2 * reg * W
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1e-1, 1e-3, 1e-4]
