...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # shape (N, C).                                                            #
    ############################################################################
    hidden = W1.T @ X.T + b1[:, None]# (H,D) @ (D,N) => (H,N)
    hidden.clamp_(0.0) # ReLU (H,N)
    scores = W2.T @ hidden + b2[:, None] # (C,H) @ (H,N) => (C,N)

    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################

    return scores.T, hidden.T
...
def nn_forward_backward(
    ...
    ############################################################################
    # y_pred = scores.argmax(dim=1) # (N,)
    scores -= scores.max(dim=1,keepdim=True).values # prevent overflow
    exp = scores.exp() # (N,C)
    S = exp.sum(dim=1, keepdim=True) # (N,) sum along classes
    probs = exp / S # (N,C)
    logprobs = -1 * probs.log()

    loss = logprobs[torch.arange(N), y].mean()
    loss += reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2)) # L2 norm
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # hidden.clamp_(0.0) # ReLU (H,N)
    # scores = W2.T @ hidden + b2[:, None] # (C,H) @ (H,N) => (C,N)
    dprobs = probs.clone() # (N,C)
    dprobs[torch.arange(N), y] -= 1
    dprobs /= N
    grads["W2"] = h1.T @ dprobs + 2 * reg * W2 # (N,H) @ (N,C) => (H,C)
    grads["b2"] = dprobs.sum(dim=0) # (C,)
    dh1 = dprobs @ W2.T # (N,C) @ (H,C) => (N,H)
    dh1[h1 <= 0] = 0 # (N,H)
    grads["W1"] = X.T @ dh1 + 2 * reg * W1 # (N,D) @ (C,H) => (D,H)
    grads["b1"] = dh1.sum(dim=0) # (H,)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def find_best_net(
    ...
    input_size = 3 * 32 * 32
    num_classes = 10
    lrs, hs, regs, lrds = get_param_set_fn()
    for lr in lrs:
        for hidden_size in hs:
            for reg in regs:
                for decay in lrds:
                    print(f"LR : {lr}\t| hidden : {hidden_size}\t| decay : {decay}\t| reg : {reg}\t")
