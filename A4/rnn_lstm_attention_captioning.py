...
def rnn_step_backward(dnext_h, cache):
    ...
    dall_act = (1 - next_h**2) * dnext_h # (N,H)
    dact = dall_act # (N,H)
    dh_act = dall_act # (N,H)
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##############################################################################
    loss = F.cross_entropy(x.permute((0,2,1)), y, reduction="sum", ignore_index=ignore_index) / x.shape[0]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        if cell_type == 'rnn':
          ...
          self.fc1 = nn.Conv2d(input_dim, hidden_dim, 1, device=device, dtype=dtype)
        else:
    ...
    def forward(self, images, captions):
        ...
        # This could be a linear layer or a CNN in the case of attention
        h0 = self.fc1.forward(features) # (N, H) or (N,H,4,4)
