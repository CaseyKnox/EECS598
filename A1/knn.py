...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    train_vec = x_train[i].flatten() # copy
    # train_vec = (train_vec - train_vec.mean()) / train_vec.std()

    for j in range(num_test):
      test_vec = x_test[j].flatten() # copy
      # test_vec = (test_vec - test_vec.mean()) / test_vec.std()

      dist_vec = test_vec - train_vec
      dist = dist_vec.pow(2).sum().sqrt()
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_test_n = x_test.reshape(num_test, -1)
  # x_test_n = (x_test_n - x_test_n.mean(axis=1).unsqueeze(1)) / x_test_n.std(axis=1).unsqueeze(1)
  for i in range(num_train):
    train_vec = x_train[i].flatten() # (C*H*W)
    # train_vec = (train_vec - train_vec.mean()) / train_vec.std()

    diff = x_test_n - train_vec # (B,C*H*W) broadcast subtraction
    dists[i] = diff.pow(2).sum(axis=1).sqrt() # shaped (B=100)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # Initialize dists to be a tensor of shape (num_train, num_test) with the
  # same datatype and device as x_train
  num_train = x_train.shape[0] # (B, C, H, W)
  num_test = x_test.shape[0] # (B2, C, H, W)
  dists = x_train.new_zeros(num_train, num_test)
  ##############################################################################
  ...
  # x_train_n = (x_train_flat - x_train_flat.mean(axis=-1).unsqueeze(1)) / x_train_flat.std(axis=1).unsqueeze(1)
  x_test_flat = x_test.reshape(num_test, -1)
  # x_test_n = (x_test_flat - x_test_flat.mean(axis=-1).unsqueeze(1)) / x_test_flat.std(axis=1).unsqueeze(1)
  x_train_n = x_train_flat
  x_test_n = x_test_flat

  # (B1, n), (B2, n) 
  # (x-y) @ (x-y).T = x @ x.T + y @ y.T - 2 y @ x.T
  # (x-y)^2 = x^2 + y^2 - 2xy
  x = x_train_n.pow(2).sum(axis=1) # (B,)
  y = x_test_n.pow(2).sum(axis=1) # (B2,)
  xy = x_train_n @ x_test_n.T # (B,n) @ (n,B2) -> (B,B2)
  dists = x.unsqueeze(1) + y - 2*xy # (B,B2)
  dists = dists.sqrt()
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, idxs = torch.topk(dists.T, k, largest=False) # (num_test, k)
  # print("idx", idxs)
  nbr_labels = y_train[idxs] # (num_test, k)
  # print("nbr_labels", nbr_labels)
  y_pred, _ = nbr_labels.mode(dim=1) # (num_test)
  # print(y_pred)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for fold in range(num_folds):
      ...
      # print(x_test)
      x_train = torch.cat(x_train_folds[:fold] + x_train_folds[fold+1:])
      y_train = torch.cat(y_train_folds[:fold] + y_train_folds[fold+1:])
      knn = KnnClassifier(x_train, y_train)
      acc = knn.check_accuracy(x_test, y_test, k)
