...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dw = xND.T @ dout # (N,D) @ (N,M) => (D,M)
        dxND = (w @ dout.T).T # (D,M) @ (N,M) => (N,D)
        assert dxND.shape == xND.shape
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params["W2"] = torch.normal(0, weight_scale, W2shape, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        N = X.shape[0]
        l1, l1_cache = Linear_ReLU.forward(X,  self.params["W1"], self.params["b1"])
        l2, l2_cache = Linear.forward(l1, self.params["W2"], self.params["b2"])
        scores = l2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        ###################################################################
        # Loss
        # y_pred = torch.argmax(r2, dim=1) # (N,)
        # assert y_pred.shape == N, f"Mismatched y_pred! Expected {N}, got {y_pred.shape}"
        loss, dx = softmax_loss(l2, y) 
        loss += self.reg * torch.sum(self.params["W1"]**2)
        loss += self.reg * torch.sum(self.params["W2"]**2)

        dout, grads["W2"], grads["b2"] = Linear.backward(dx, l2_cache)
        dx,   grads["W1"], grads["b1"] = Linear_ReLU.backward(dout, l1_cache)
        grads["W2"] += 2*self.reg*l2_cache[1] # 2 * reg * W2
        grads["W1"] += 2*self.reg*l1_cache[0][1] # 2 * reg * W1
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          shape = (last_dim, all_dims[i])
          self.params[f"W{i+1}"] = torch.normal(0, weight_scale, shape, dtype=dtype, device=device)
          self.params[f"b{i+1}"] = torch.zeros(all_dims[i], dtype=dtype, device=device)
          last_dim = all_dims[i]
          print(f"W{i+1} shape : ", shape)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for layer in layers:
          ...
          if "Dropout" in layer.__name__:
            out, cache = layer.forward(out, self.dropout_param)
          else:
            i += 1
            out, cache = layer.forward(out, W, b)
          caches.append(cache)
        scores = out
        #################################################################
        #                      END OF YOUR CODE                         #
        ...
        #     if i == self.num_layers-1:
        #       dx, grads[f"W{i+1}"], grads[f"b{i+1}"] = Linear.backward(dx, caches[i])
        #     else:
        #       dx, grads[f"W{i+1}"], grads[f"b{i+1}"] = Linear_ReLU.backward(dx, caches[i])
        #     grads[f"W{i+1}"] += self.reg * W

        i -= 1
        for cache, layer in zip(caches[::-1], layers[::-1]):
          W = self.params[f"W{i+1}"]
          if "Dropout" in layer.__name__:
              dx = layer.backward(dx, cache)
          elif "Linear" in layer.__name__:
              dx, grads[f"W{i+1}"], grads[f"b{i+1}"] = layer.backward(dx, cache)
              grads[f"W{i+1}"] += self.reg * W
              i -= 1
          else:
              print(f"Unknown layer {layer.__name}")
...
def create_solver_instance(data_dict, dtype, device):
    ...
    #############################################################
    # lr=1.0, batch=512, decay=0.98
    # lr=1.5, batch=1024, decay=0.9 
    ...
    # ---------
    # 52%
    # solver = Solver(model, data_dict, optim_config={ "learning_rate" : 1.0, "reg" : 1e-5,}, lr_decay=0.98,batch_size=1024,num_epochs=50,device=device)
    # 45%
    # solver = Solver(model, data_dict, optim_config={ "learning_rate" : 1.0, "reg" : 1e-5,}, lr_decay=0.98,batch_size=2048,num_epochs=50,device=device)
    # 
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
...
def rmsprop(w, dw, config=None):
    ...
    g2 = decay * cache + (1 - decay) * dw * dw
    n = dw / (torch.sqrt(g2) + eps)
    next_w = w - lr * n
...
def adam(w, dw, config=None):
    ...
    next_w = w - (lr * first_unbias) / (torch.sqrt(second_unbias) + eps)
