...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        for r in range(0, x_pad.shape[-2]-2*pad, stride):
          for c in range(0, x_pad.shape[-1]-2*pad, stride):

            # convolve for each image pixel
            val = torch.zeros((F,N), device=x.device, dtype=x.dtype)
            for hh in range(HH):
              for ww in range(WW):
                val += w[:,:, hh, ww] @ x_pad[:,:, r+hh, c+ww].T # (F,C) @ (N,C) => (F,N)
            
            # save output
            out[:,:,r//stride,c//stride] = val.T + b
    ...
    def backward(dout, cache):
        ...
        dw = torch.zeros_like(w)               # shape (F,C,HH,WW)
        dx_pad = torch.zeros_like(x_pad)       # shape (N,C,H+2p,W+2p)

        # loop over every example, filter, and output cell
        for n in range(N):
          for f in range(F):
            for i in range(H_out):
              for j in range(W_out):
                # compute window corners in padded input
                i0 = i * stride
                j0 = j * stride

                # slice out the (CxHHxWW) patch
                x_window = x_pad[n, :, i0:i0+HH, j0:j0+WW]  # (C,HH,WW)
                
                # upstream gradient for this output
                d_out = dout[n, f, i, j]                   # scalar

                # accumulate into dw[f]
                # broadcast: (C,HH,WW) * scalar -> (C,HH,WW)
                dw[f] += d_out * x_window

                # accumulate into dx_pad at the same window
                # w[f] has shape (C,HH,WW)
                dx_pad[n, :, i0:i0+HH, j0:j0+WW] += d_out * w[f]

        # unpad to get dx of shape (N,C,H,W)
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
          for c in range(C):
            for h in range(HH):
              for w in range(WW):
                # start at the top-left of this pool window
                mm = x[n, c, h*stride, w*stride]
                # slide over the window of size (ph x pw)
                for i in range(ph):
                  for j in range(pw):
                    val = x[n, c, h*stride + i, w*stride + j]
                    if val > mm:
                      mm = val
                out[n, c, h, w] = mm
                ...
                for i in range(ph):
                  for j in range(pw):
                    ...
                    if val > mm:
                      mm = val
                      ij = (i,j)
                dx[n, c, h*stride + ij[0], w*stride + ij[1]] = dout[n,c,h,w]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        H1shape = (F * H//2 * W//2, hidden_dim) # assume conv outputs same image size & 2x2 pooling
        H2shape = (hidden_dim, num_classes)
        self.params["W1"] = torch.normal(0, weight_scale, W1shape, dtype=dtype, device=device)
        self.params["b1"] = torch.zeros(F, dtype=dtype, device=device)
        self.params["W2"] = torch.normal(0, weight_scale, H1shape, dtype=dtype, device=device)
        self.params["b2"] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        self.params["W3"] = torch.normal(0, weight_scale, H2shape, dtype=dtype, device=device)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i, F in enumerate(num_filters):
          shape = (F, C, 3, 3) # (N, C, H, W) x (F, C, H, W) -> (N, F, H, W)
          if weight_scale == "kaiming":
            self.params[f"W{i+1}"] = kaiming_initializer(C, F, 3, device=device, dtype=dtype)
          else:
            self.params[f"W{i+1}"] = torch.normal(0, weight_scale, shape, dtype=dtype, device=device)
        ...
        shape = (C * H * W, num_classes)
        if weight_scale == "kaiming":
          self.params[f"W{self.num_layers}"] = kaiming_initializer(Din=C*H*W, Dout=num_classes, device=device, dtype=dtype)
        else:
          self.params[f"W{self.num_layers}"] = torch.normal(0, weight_scale, shape, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          # select layer forward function (max pool, batchnorm, etc)
          if i in self.max_pools:
            if self.batchnorm:
              func = Conv_BatchNorm_ReLU_Pool.forward
              args = (out, w, b, gamma, beta, conv_param, self.bn_params[i], pool_param)
              backward_funcs.append(Conv_BatchNorm_ReLU_Pool.backward)
            else:
              func = Conv_ReLU_Pool.forward
              args = (out, w, b, conv_param, pool_param)
              backward_funcs.append(Conv_ReLU_Pool.backward)
            H = H//2
            W = W//2
          else:
            if self.batchnorm:
              func = Conv_BatchNorm_ReLU.forward
              args = (out, w, b, gamma, beta, conv_param, self.bn_params[i])
              backward_funcs.append(Conv_BatchNorm_ReLU.backward)
            else:
              func = Conv_ReLU.forward
              args = (out, w, b, conv_param)
              backward_funcs.append(Conv_ReLU.backward)
          
          # run forward pass and reshape output
          out, cache = func(*args)
        ...
        backward_funcs.append(Linear.backward)
        caches.append(cache)
        ...
        for i in range(self.num_layers-1, -1, -1):
          func = backward_funcs[i]
          cache = caches[i]
          if i == self.num_layers-1: # fully connected
            x,w,b = cache
            dx, dw, db = Linear.backward(dx, cache)
          else:
            conv_cache = cache[0]
            x,w,b,_,_,_,_ = conv_cache
          
            if self.batchnorm:
              dx, dw, db, grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = func(dx, cache)
            else:
              dx, dw, db = func(dx, cache)

            
          # elif len(cache) == 3: # conv relu pool
          #   conv_cache = cache[0]
          #   x,w,b,_,_,_,_ = conv_cache
          #   if self.batchnorm:
          #     dx, dw, db, grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = Conv_BatchNorm_ReLU_Pool.backward(dx, cache)
          #   else:
          #     dx, dw, db = Conv_ReLU_Pool.backward(dx, cache)
          # elif len(cache) == 2: # conv relu
          #   conv_cache = cache[0]
          #   x,w,b,_,_,_,_ = conv_cache
          #   if self.batchnorm:
          #     dx, dw, db, grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = Conv_BatchNorm_ReLU.backward(dx, cache)
          #   else:
          #     dx, dw, db = Conv_ReLU.backward(dx, cache)
          # else:
          #   print(f"No way to handle cache with length {len(cache)}")
          #   raise ValueError(f"Invalid cache. len: {len(cache)}")
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        shape = (Din, Dout)
        std = 1
        weight = torch.normal(0, std, shape, device=device, dtype=dtype)
        weight *= (gain / Din)**0.5
        ...
        std = 1
        in_channels = Din*K*K
        weight = torch.normal(0, 1, shape, device=device, dtype=dtype)
        weight *= (gain / in_channels)**0.5
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            ##################################################################
            assert x.shape[1] == gamma.shape[0], f"x.shape is {x.shape}. Gamma shape is {gamma.shape}"
            assert x.shape[1] == beta.shape[0], f"x.shape is {x.shape}. Beta shape is {beta.shape}"
            sample_mean = x.mean(dim=0)
            sample_var = x.var(dim=0, unbiased=False)
            x_mu = (x - sample_mean)
            inv_std = 1 / torch.sqrt(sample_var + eps)
            x_hat = x_mu * inv_std # normalize
            out = x_hat * gamma + beta # scale and shift

            running_mean = momentum * running_mean + (1 - momentum) * sample_mean
            running_var = momentum * running_var + (1 - momentum) * sample_var
            cache = (x, x_mu, inv_std, x_hat, sample_mean, sample_var, gamma, beta, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            # in the out variable.                                         #
            ################################################################
            out = (x - running_mean) / torch.sqrt(running_var + eps) # normalize with running mean/var
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        x, x_mu, inv_std, x_hat, sample_mean, sample_var, gamma, beta, eps = cache
        N = x.shape[0]
        ...
        dx_mu = inv_std * dx_hat # (N,D)
        dinv_std = (x_mu * dx_hat).sum(dim=0) # (D,)
        dvar = dinv_std * -0.5 * (sample_var + eps)**(-3/2)
        dx_mu += dvar * (2.0 / N) * x_mu
        dmu = -1 * dx_mu.sum(dim=0)
        dx = dx_mu + dmu / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        out, cache = BatchNorm.forward(x0.reshape(-1,C), gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        N,C,H,W = dout.shape
        dout0 = dout.permute((0,2,3,1)).reshape((-1,C))
        dx, dgamma, dbeta = BatchNorm.backward(dout0, cache)
