...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # Numerator is the same for sim(z_k, z_k_N) and sim(z_k_N, z_k)
        numerator = torch.exp(sim(z_k, z_k_N) / tau)
        denom_l = 0
        denom_r = 0
        for l in range(2*N):
            if l != k:
                denom_l += torch.exp(sim(z_k, out[l]) / tau)
            if l != k+N:
                denom_r += torch.exp(sim(z_k_N, out[l]) / tau)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denominator = torch.exp(sim_matrix / tau) # (2N, 2N)
    
    # Mask out terms where k=i.
    denominator.fill_diagonal_(0.)
    
    # Hint: Compute the denominator values for all augmented samples. This should be a 2N x 1 vector.
    denominator = denominator.sum(dim=1, keepdim=True) # (2N,1)
    ...
    numerator = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numerator = torch.tile(torch.exp(similarity / tau), (2,1)) # (2N, 1)

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = -torch.log(numerator[:N] / denominator[:N])
    loss += -torch.log(numerator[N:] / denominator[N:])
    loss /= 1 / (2 * N)
